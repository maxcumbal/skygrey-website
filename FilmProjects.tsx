    ...
    <section id="projects" ref={sectionRef} className="relative bg-black">
      ...
      <div className="py-16 px-4 md:py-20 md:px-6">
        <div className="container mx-auto max-w-7xl mb-12 md:mb-20">
          <h2
            ref={titleRef}
            className="text-4xl sm:text-5xl md:text-7xl tracking-wider text-center"
        ...
        <div className="container mx-auto max-w-7xl">
          {/* Desktop: 2-Spalten sticky Layout */}
          <div className="hidden lg:grid lg:grid-cols-2 gap-12 items-start">
            {/* Links: Sticky Video */}
            <div className="lg:sticky lg:top-24 flex justify-center">
              <div
                className="relative overflow-hidden"
                style={{ borderRadius: '16px', backgroundColor: '#000', width: '100%', maxWidth: '350px', aspectRatio: '9 / 16' }}
              >
            ...
            {/* Rechts: Scrollende Beschreibungen */}
                ...
                >
                  <div className={`transition-all duration-700 ${activeIndex === index ? 'opacity-100 translate-y-0' : 'opacity-40 translate-y-8'}`}>
                    ...
                    <h3 className="text-5xl lg:text-6xl mb-8 leading-tight"
                      style={{ color: activeIndex === index ? 'white' : 'rgba(255,255,255,0.5)' }}>
                      {project.title}
                    </h3>
                    <p className="text-lg leading-relaxed mb-10 max-w-xl"
                      style={{ color: activeIndex === index ? 'rgba(255,255,255,0.8)' : 'rgba(255,255,255,0.4)' }}>
                      {project.description}
                    </p>
              ...
              >
                ...
                <div className="flex justify-center mb-6">
                  <div className="relative overflow-hidden w-full"
                    style={{ borderRadius: '16px', backgroundColor: '#000', maxWidth: '280px', aspectRatio: '9 / 16' }}>
                    <iframe
                      src={project.video}
                      style={{
                        position: 'absolute', top: 0, left: 0,
                ...
                <div className="px-2">
                  ...
                  <p className="text-sm sm:text-base leading-relaxed mb-6 text-white/70">{project.description}</p>
      ...
      {/* ===== ÜBERSCHRIFT ===== */}
      <div className="py-16 md:py-24 px-4 md:px-6 text-center bg-black">
        <h2 className="text-3xl sm:text-5xl md:text-7xl lg:text-8xl font-light tracking-widest text-white mb-6">
          DEINE VISION. <span style={{ color: 'var(--gold)' }}>UNSER BILD.</span>
        </h2>
        <div className="w-24 h-[1px] mx-auto" style={{ backgroundColor: 'var(--gold)' }} />
      </div>

      {/* ===== TEIL 2: Horizontale Fullscreen Videos ===== */}
      <div className="relative w-full overflow-hidden" style={{ height: '100svh' }}>
        ...
        {/* Pfeil-Buttons links/rechts (Desktop) */}
        <button
          onClick={prevVideo}
          className="absolute left-4 md:left-8 top-1/2 -translate-y-1/2 hidden md:flex items-center justify-center w-12 h-12 rounded-full transition-all"
          style={{ zIndex: 3, background: 'rgba(0,0,0,0.5)', border: '1px solid rgba(183,148,92,0.4)' }}
        >
          <ChevronLeft size={24} style={{ color: 'var(--gold)' }} />
        </button>
        <button
          onClick={nextVideo}
          className="absolute right-4 md:right-8 top-1/2 -translate-y-1/2 hidden md:flex items-center justify-center w-12 h-12 rounded-full transition-all"
          style={{ zIndex: 3, background: 'rgba(0,0,0,0.5)', border: '1px solid rgba(183,148,92,0.4)' }}
        >
          <ChevronRight size={24} style={{ color: 'var(--gold)' }} />
        </button>

        {/* Punkte Navigation (alle Geräte) */}
        <div className="absolute bottom-8 left-0 right-0 flex justify-center gap-3" style={{ zIndex: 3 }}>
          {horizontalVideos.map((_, index) => (
            <button
              key={index}
              onClick={() => setActiveHorizontal(index)}
              style={{
